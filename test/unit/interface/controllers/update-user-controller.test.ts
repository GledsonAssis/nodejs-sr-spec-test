...
import { PutUserController } from "@/interfaces/controllers";

jest.mock("@/application/usecases", () => {
  return {
    PutUser: jest.fn().mockImplementation(() => ({
      execute: jest.fn(),
    })),
  };
});
...
describe("PutUserController", () => {
  let createUserController: PutUserController;
  let mockUsecase: jest.Mocked<PutUser>;
  let mockInputValidator: jest.Mocked<InputValidator>;

  beforeEach(() => {
    mockUsecase = new (require("@/application/usecases").PutUser)();

    createUserController = new PutUserController();
